...
namespace VertecoModel
{
    class LBNL_Forecast_File :LBNL_Base_File 
    {
        ...
        public ModelBuilding Building { get; set; }
...
#region public methods
        ...
        public bool WriteToFile()
        {
            ...
            try
            {
                ...
                sw.WriteLine("# Training parameters used:");
                sw.WriteLine("# -------------------------");
                sw.WriteLine("# Training Period: from {0} to {1}", Building.TrainingStartMMDD.ToString("D4"), Building.TrainingEndMMDD.ToString("D4"));
                // working Days is a little complex
                string workingDaysAsString = "\t# Working Days: ";
                foreach (DayOfWeek day in Building.WorkingDays)
                {
                    workingDaysAsString += day.ToString() + "/";
                }
                sw.WriteLine(workingDaysAsString );
                
        

                sw.WriteLine("\t# Working Day end:" + this.ForecastModel.WorkingDayEndTime.ToShortTimeString());
                sw.WriteLine("#");
                sw.WriteLine("# Model Information");
                sw.WriteLine("# -----------------");
